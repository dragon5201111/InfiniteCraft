...
class ElementManager extends ElementSubject{
    ...
    addElement(element){
        ...
        element.getDiv().addEventListener("click", () => this.notify(element, ELEMENT_DIV_EVENTS.CLICKED));
        this.notify(element, ELEMENT_DIV_EVENTS.ADDED);
    }
...
class ElementHistory extends Storable{
    ...
    update(subject, element, eventType){
        if (eventType != ELEMENT_DIV_EVENTS.ADDED){
            return;
        }
...
class ElementFuser {
    ...
    // Observer method
    update(subject, element, eventType){
        if (eventType != ELEMENT_DIV_EVENTS.CLICKED){
            return;
        }
